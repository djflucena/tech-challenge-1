...
class TestComportamentoImportacaoService(unittest.TestCase):
    def setUp(self):
        self.ano = 2023

    @patch("src.services.importacao_service.ImportacaoRaspagem")
    @patch("src.services.importacao_service.RawRepository")
    @patch("src.services.importacao_service.ImportacaoRepository")
    ...
    @patch("src.services.importacao_service.ImportacaoRaspagem", side_effect=TimeoutRequisicao())
    @patch("src.services.importacao_service.ImportacaoRepository")
    def test_quando_timeout_entao_retorna_dados_locais(self, mock_repo_final, _):
        """Cenário: Timeout na raspagem"""
        service = ImportacaoService()
        with patch.object(service.importacao_repository, "get_por_ano", return_value="dados locais") as fallback:
